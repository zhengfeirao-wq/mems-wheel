...
### 1. ~~`fresh_mask` 恒为 0~~ —— **已在 v3 固件修复**

> **状态更新（2026-09-11）：已修复并实机验证。** 四板 88 个通道全部恢复，
> `init_error` 归零，`fresh ≈ 0.5`（健康值）。详见
> [`firmware_tactile500/docs/DRDY修复与v3自愈记录.md`](firmware_tactile500/docs/DRDY修复与v3自愈记录.md)。
> 以下保留原分析作为背景。

原固件中 100% 的帧带 `not_all_fresh` 状态，`per_channel_fresh_fraction` 全为 0。
这意味着固件的 DRDY 轮询取不到「新转换」证据。
...
**根因**：原固件初始化末尾写 `0xA5 = 0x88`，把 bit7 `DAC_on` 置 1。
...
另有一个叠加因素：板载 EEPROM 被早期固件烧入了 `0xA5 = 0x88`，
上电自动加载后若固件的 `0xA5 = 0x08` 写入未生效，芯片会退回模拟输出模式，
此时 `0x30` 触发命令被忽略 —— v3 用**运行中自愈**（连续 4 帧 DRDY=0 即重写配置）解决。

**修复要点**：保持 `DAC_on=0`，改用 `0x30` COMMAND 寄存器触发单次转换，
做流水线采集（读完即触发下一轮，转换在帧间隔内完成），SPI 提至 3.75 MHz。
**协议与上位机均无需改动。**

### 2. `fresh ≈ 0.5` 是真实转换能力的反映，不是缺陷

NSA2302 在单次转换模式下一次完整转换（含每次 power-up）耗时 **2~4 ms**。
在 500 Hz 帧率下，因此每两帧才有一个真实新测量 —— `fresh` 比例约为 **0.5**。

这解释了早期"模拟输出模式"为何能到 ~489 次/秒：那是**自主连续转换、无每次上电开销**，
但代价是 DRDY 永不置位。**两者是本质权衡。**

若要每帧都是真实新测量，可把帧率降到 250 Hz；当前设计保留 500 Hz 帧率并用
`fresh_mask` 如实标记哪一帧是新数据。
